            ...
            ajaxForm_list.each(function () {
                $(this).validate({
                    ...
                    submitHandler:function (form) {
                        var realSubmitHandler=function (form) {
                        var $form = $(form);
                        $form.ajaxSubmit({
                    ...
                    var that = this;
                        if(!$btn.data('qq_captcha_app_id')){
                            return realSubmitHandler.apply(that,arguments);
                        }
                         var $form = $(form);
                         if($form.children('[name="ticket"]').length==0){
                            $form.append('<input name="ticket" type="hidden" value="" />');
                            $form.append('<input name="randstr" type="hidden" value="" />');
                         }
                        var tencentCaptcha = new TencentCaptcha($btn.data('qq_captcha_app_id')+'', function (res) {
                            if (res.ret !== 0) {
                                return false;
                            }


                            $form.children('[name="ticket"]').val(res.ticket);
                            $form.children('[name="randstr"]').val(res.randstr);
                            realSubmitHandler.call(that,form);
